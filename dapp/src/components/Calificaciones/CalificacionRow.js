...
const CalificacionRow = (props) => {
    const {drizzle, drizzleState,
                alumnoIndex, alumnoAddr, evaluacionesLength} = props;
    let cells = [];
    let notaFinal = 0;
            ...
            <ContractData
                ...
                render={nota => <>
                    <td key={"p2-" + alumnoIndex + "-" + ei}>
                        {nota.tipo === "0" ? "N.P." : ""}
                        {nota.tipo === "1" ?
                            (nota.calificacion / 10).toFixed(1) : ""}
                        {nota.tipo === "2" ?
                            (nota.calificacion / 10).toFixed(1) + "(M.H.)" : ""}
                    </td>
 {/*                    <td>
                        <ContractData
                            drizzle={drizzle}
                            ...
                            contract={"Asignatura"}
                            method={"evaluaciones"}
                            render={eval => notaFinal += (eval.puntos/10) * nota.calificacion}
                        />
                    </td> */}
                 </>                
                }
